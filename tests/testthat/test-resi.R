...
test_that("data is needed for certain models", {
  # Explicitly exercise the bootstrap path, which requires model data.
...
test_that("RESI estimates are in between the confidence limits", {
  resi.obj = resi(mod, nboot = 500, store.boot = TRUE, ci.method = "boot")
  ...
  # Keep the bootstrap method explicit because these tests exercise stored draws.
