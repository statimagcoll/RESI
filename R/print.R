...
print.resi <- function(x, digits = max(3L, getOption("digits") - 3L), ...) {
  ...
  ci_method <- if (!is.null(x$ci.method)) x$ci.method else "boot"
  if (ci_method == "boot") {
    if (!(is.null(x$nboot))){
  ...
  } else if (ci_method == "qf") {
    cat("\n2. Confidence intervals were constructed using the quadratic form method.\n")
  } else if (ci_method == "cf") {
    cat("\n2. Confidence intervals were constructed using the Cornish-Fisher method.\n")
  } else if (ci_method == "normal") {
    cat("\n2. Confidence intervals were constructed using the Gaussian (normal) method.\n")
  }
